...
def decode():
    with tf.Session() as sess:
        ...
        while sentence:
            ...
            token_ids = data_util.sentence_to_token_ids(tf.compat.as_bytes(sentence), nl_vocab)
            ...
            print(" ".join([tf.compat.as_str(rev_name_vocab[output]) for output in outputs]))
            print("> ", end="")
            sys.stdout.flush()
            sentence = sys.stdin.readline()
...
def main(_):
    data_util.create_set(FLAGS.data_dir)
    if FLAGS.self_test:
        self_test()
    elif FLAGS.decode:
        decode()
