...
class S2SModel(object):
    ...
    def get_batch(self, bucket_dbs, bucket_id):
        ...
        for _ in range(self.batch_size):
            ...
            encoder_input = data_utils.sentence_indice_ask(encoder_input)
            decoder_input = data_utils.sentence_indice_answer(decoder_input)
            # Encoder
            encoder_pad = [data_utils.ASK_PAD_ID] * (encoder_size - len(encoder_input))
            encoder_inputs.append(list(reversed(encoder_input + encoder_pad)))
            # Decoder
            decoder_pad_size = decoder_size - len(decoder_input) - 1
            decoder_inputs.append(
                [data_utils.ANSWER_GO_ID] + decoder_input +
                [data_utils.ANSWER_PAD_ID] * decoder_pad_size
            )
        ...
        for i in range(decoder_size):
            ...
            for j in range(self.batch_size):
                ...
                if i == decoder_size - 1 or target == data_utils.ANSWER_PAD_ID:
                    batch_weight[j] = 0.0
